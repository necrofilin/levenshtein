...
void levenshtein_reset_pattern(Levenshtein *levenshtein, zval *object) {
    ...
    pattern = zend_read_property(levenshtein_ce, object, "pattern", sizeof("pattern") - 1, 1 TSRMLS_CC);
    switch (pattern->type) {
        case IS_STRING:
            levenshtein->setPattern(Z_STRVAL_P(pattern));
            levenshtein_update_pattern(levenshtein, object);
...
PHP_METHOD (Levenshtein, getStorage) {
    zval *object = getThis();
    zval *val = zend_read_property(levenshtein_ce, object, "storage", sizeof("storage") - 1, 0 TSRMLS_CC);
    *return_value = *val;
    zval_copy_ctor(return_value);
}